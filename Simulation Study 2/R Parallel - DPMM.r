#Note: since for the DPMM model we use 20 classes more than the BLC, please use data frame which include the number of classes selected for the BLC model
...
clusterExport(cl,c("dset","classes","npar","vobs","mm","IT1","IT2","IT3","ITPRINT","PRIRESP"))
...
ptm <- proc.time()
results<-foreach(i=1:B,.combine=comb,.multicombine=TRUE,.init=list(list(),list(),list(),list()),.packages=c('DPMMimpute')) %dopar% {
	...
	a<-dpmi(Y=dset[[i]],K=classes[i],alpha=1,prioresp=PRIRESP,c=0.25,d=0.25,I=mm,it1=IT1,it2=IT2,it3=ITPRINT,v=10,estimates=FALSE,probs=FALSE,supit=FALSE,type=1,plug=TRUE,infoclass=FALSE,rand=TRUE,count=FALSE)
	
	
	
	
	for(j in 1:mm){
		moda<-glm(as.factor(Y)~X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13+X14+X15+X16+X17+X18+X19+X20+X1:X5+X1:X17+X1:X5:X17,family='binomial',data=a[[2]][[j]])
